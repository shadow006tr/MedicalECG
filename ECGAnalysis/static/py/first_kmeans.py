...
# 5. check for each pulse if the pulse is a noise or not by 2 criterions and then construct a new matrix with no noises
...
def detectNoisesWithRange(startILeft, endILeft, startIRight, endIRight, leadValues):
    ...
    for i in range(startIRight + 1, endIRight + 1):
        ...
        if leadValues[i] == leadValues[i - 1]:
            countZeroes = countZeroes + 1
    # print("zeroes counter =", countZeroes, leadValues[startILeft], leadValues[endILeft], leadValues[startIRight], leadValues[endIRight])
